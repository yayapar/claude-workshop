...
class TestExtractTags:
    def test_finds_tags_and_preserves_order(self) -> None:
    ...
    def test_headings_are_not_tags(self, text: str) -> None:
    ...
    def test_numeric_reference_is_not_a_tag(self) -> None:
        assert extract_tags("fixes #123") == []

    def test_alphanumeric_tag_is_allowed(self) -> None:
    ...
    def test_tilde_fenced_code_is_skipped(self) -> None:
        assert extract_tags("~~~\n#hidden\n~~~\n#kept") == ["kept"]

    def test_shorter_inner_fence_does_not_close_a_longer_outer_fence(self) -> None:
        assert extract_tags("````\n```\n#hidden\n```\n````\n#kept") == ["kept"]

    def test_longer_closing_fence_is_allowed(self) -> None:
        assert extract_tags("```\n#hidden\n``````\n#kept") == ["kept"]
    ...
    def test_unclosed_fence_swallows_rest_of_file(self) -> None:
        assert extract_tags("#kept\n```\n#hidden\n") == ["kept"]

    def test_inline_code_is_skipped(self) -> None:
        assert extract_tags("use `#hidden` but keep #kept") == ["kept"]

    def test_nested_tags_are_kept_whole(self) -> None:
    ...
    def test_surrounding_punctuation_is_trimmed(self, text: str, expected: str) -> None:
        assert extract_tags(text) == [expected]

    def test_hyphens_and_underscores_are_kept(self) -> None:
        assert extract_tags("#in-progress #needs_review") == ["in-progress", "needs_review"]

    def test_tags_are_case_sensitive(self) -> None:
...
class TestIterMarkdown:
    def test_finds_markdown_recursively_and_ignores_other_files(self) -> None:
    ...
    def test_a_dotted_root_is_still_scanned(self, tmp_path: Path) -> None:
    ...
    def test_missing_path_raises_eagerly(self, tmp_path: Path) -> None:
...
class TestCountTags:
    def test_counts_across_the_example_notes(self) -> None:
    ...
    def test_counts_a_single_file(self) -> None:
        assert count_tags(NOTES / "basic.md") == {"python": 3, "testing": 2}

    def test_file_without_tags(self) -> None:
    ...
    def test_hidden_directories_contribute_nothing(self, tmp_path: Path) -> None:
    ...
    def test_byte_order_mark_does_not_swallow_the_first_tag(self, tmp_path: Path) -> None:
        note = tmp_path / "bom.md"
        note.write_bytes(b"\xef\xbb\xbf#alpha and #beta\n")
        assert count_tags(note) == {"alpha": 1, "beta": 1}

    def test_undecodable_note_does_not_abort_the_scan(self, tmp_path: Path) -> None:
...
class TestNotesTags:
    def test_prints_each_tag_with_its_count_sorted(self, tmp_path: Path) -> None:
    ...
    def test_columns_are_aligned(self, tmp_path: Path) -> None:
    ...
    def test_returns_the_counts(self, tmp_path: Path) -> None:
        (tmp_path / "note.md").write_text("#alpha #alpha\n")
        assert notes_tags(tmp_path, file=io.StringIO()) == {"alpha": 2}

    def test_prints_nothing_when_there_are_no_tags(self, tmp_path: Path) -> None:
...
class TestMain:
    def test_scans_the_given_path(self, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    def test_missing_path_reports_an_error(
    ...
    def test_undecodable_note_does_not_raise(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
    ...
    def test_unreadable_directory_reports_an_error(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
